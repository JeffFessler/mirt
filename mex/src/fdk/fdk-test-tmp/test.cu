...
#if 0 // the lines below cause crash. cpu should not address via device pointers!
...
int nrhs, Const mxArray *prhs[])
{
	...
	if (Streq(arg, "fdk,st,back") || Streq(arg, "fdk,ts,back")) {
		cjool is_st = Streqn(arg, "fdk,st", 6);
		if (nrhs != 13 || nlhs != 1)
			Fail(Usage)
...
//
static jool fdk_ts_back_init(void *in, cint id, cint nthread)
{
...
cjool chat)
{
	...
	const int nj_per_thread = (int) ceil(sum / (rscalar) nthread);
	if (chat > 99)
...
cint	nz,
cdouble	dx,
cdouble	dy,		/* can be negative to cause flip */
cdouble	dz,
cdouble	offset_x,	/* center offset in pixels (usually 0) */
cdouble	offset_y,
cdouble	offset_z,
...
#endif
cdouble	dso,	/* distance from source to isocenter */
cdouble	dsd,	/* distance from source to detector */
cint	ns,
cint	nt,
cdouble	ds,	/* horizontal ray spacing */
cdouble	dt,	/* vertical ray spacing */
cdouble	offset_s,	/* channel offset [pixels] */
cdouble	offset_t,	/* vertical offset on detector [pixels] */
cfloat	*proj,	/* [ns,nt] projection view at angle beta */
cdouble	beta,	/* source angle [radians] */
cjool	is_arc)
{
	int ix, iy, iz;
	cint nxy = nx * ny;
	cdouble wx = (nx-1)/2. + offset_x;
	cdouble wy = (ny-1)/2. + offset_y;
	cdouble wz = (nz-1)/2. + offset_z;
	cdouble ws = (ns-1)/2. + offset_s;
	cdouble wt = (nt-1)/2. + offset_t;
	cdouble sinb = sin(beta);
	cdouble cosb = cos(beta);
	...
	for (iy = 0; iy < ny; ++iy) {
	 ...
	 for (ix = 0; ix < nx; ++ix, ++image) {
	  ...
	  if (!mask2[ix + iy*nx])
		...
		cdouble xx = dx * (ix - wx);
		cdouble xbeta = xx * cosb + yy * sinb;
		cdouble ybetas = dso - (-xx * sinb + yy * cosb); 
		cdouble mag = dsd / ybetas; 
		cdouble ss = is_arc ? (dsd * atan2(xbeta, ybetas))
				: (mag * xbeta);
		cdouble ss_bin = ss / ds + ws;
		cint is = floor(ss_bin);

		if (is < 0 || is >= ns-1) /* out of FOV */
			continue;

		else {
			cdouble wr = ss_bin - is;
			cdouble wl = 1. - wr;
			cdouble w2 = is_arc ?
                        (Sqr(dsd) / (Sqr(ybetas) + Sqr(xbeta))) : Sqr(mag);
			...
			for (iz = 0; iz < nz; ++iz) {
				cdouble zz = dz * (iz - wz);
				cdouble tt = mag * zz;
				cdouble tt_bin = tt / dt + wt;
				cint it = floor(tt_bin);

				if (it < 0 || it >= nt-1) /* out of FOV */
					continue;
				else {
					cdouble wu = tt_bin - it;
					cdouble wd = 1. - wu;
					cdouble p1 = wl * proj[is + it*ns]
						+ wr * proj[is+1 + it*ns];
					cdouble p2 = wl * proj[is + (it+1)*ns]
...
*/
static jool jf_thread1_setaffinity_mac(cint ithread)
...
*/
jool jf_thread1_affinity_check(cint chat)
{
#if Use_nptl
	if (chat)
...
*/
static jool jf_thread1_setaffinity_attr(
pthread_attr_t *attr,
cint ithread,
Const jf_thread1_affinity *aff, // affinity control
cint chat)
...
* mxu_showdim()
*/
static jool mxu_showdim(Const mxArray *mx)
{
	int id, ndim;
	Const mwSize *dims;
...
* Show arguments
*/
jool mxu_arg(cint nmx, Const mxArray *pmx[])
{
	int ii;
