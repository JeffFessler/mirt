...
static int helix_rebin_na_half(
cfloat dso,
cfloat dsd,
cfloat dfs,
cfloat ds,
...
{
	cfloat num_turns = orbit / 360.;
	cint num_betas = floorf(na/num_turns);
	cfloat mypitch = pitch * nt * dso / dsd * dt;
	cfloat ws = (ns-1)/2. + offset_s;
	cfloat s0 = ds * (0 - ws);
	cfloat s1 = ds * (ns-1 - ws);
	cfloat smax = Max(s0, s1);
	...
	cfloat delta = asin(rmax/dso);
	cfloat dist = 0.5 * mypitch * (M_PI + 2*delta) / (2*M_PI);

	cint na_half = floorf(2 * dist / mypitch * num_betas);

//	*p_num_betas = num_betas;
...
static void helix_rebin_mex_help(void)
{
	...
	printf("\n\
\n\
	[sino z_orbit_start] = function('helix,rebin,ssrb',\n\
		nz, dz, offset_z, \n\
		dso, dsd, dfs, [ds dt], [offset_s offset_t], \n\
		pitch, source_z0, orbit, orbit_start, proj, nthread)\n\
\n\
		output sinogram output is single [ns na_rebin nz]\n\
		output z_orbit_start is single [nz]\n\
...
//	int num_betas;
	cint na_rebin = helix_rebin_na_half(dso, dsd, dfs, ds, dt,
		offset_s, pitch, orbit, ns, nt, na);

	mwSize odim[3] = {ns, na_rebin, nz}; // usual order
	Call(plhs[0] = mxCreateNumericArray, (3, odim, mxSINGLE_CLASS, mxREAL))

	Call(plhs[1] = mxCreateNumericMatrix, (nz, 1, mxSINGLE_CLASS, mxREAL));
	...
	Call(helix_rebin_ssrb, (
			...
			(float *) mxGetPr(plhs[1]), // z_orbit_start [nz]
			na_rebin,
			nz,
			mxGetSingle(mx_dz),
