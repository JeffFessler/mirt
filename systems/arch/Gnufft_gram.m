...
if ~isreal(wi)
	...
	fail 'todo complex: probably there is anti-Hermitian-ness to consider'
end
...
if ~isreal(wi)
	...
	kern = kern + 1i * ikern; % since everything is linear
	fail('todo: consider hermitian symmetry or lack thereof')
end

%clf, im(real([kern; t; t-kern]))
%clf, im(imag([kern; t; t-kern]))
%printm('me vs hugo %g%%', max_percent_diff(t, kern))
...
function A = build_Gmod1(arg)
st = arg.st;
if ~isfield(st, 'phase_shift'), error 'only table-nufft done', end
...
function A = build_Gmod2(arg)
st = arg.st;
if ~isfield(st, 'phase_shift'), error 'only table-nufft not done', end
...
A = Gnufft(mask, arg); % todo: can i avoid this?
%}
...
if 0 % in 1D insignificant residual error!
	[t0 t1] = test_adjoint(T, 'complex', 1, 'warn', 1);
	err = (t0 - t1') / max(abs(t0(:)));
	im clf, im(err)
return
end
...
% rng(0)
% todo wi = wi + 1i*randn(size(wi)); % stress it with complex case!

x = ellipse_im(ig, 'shepplogan-emis');

% todo: build Gdsft here for comparison
...
for ic=1:numel(classes)
	...
	fatrix2_tests(T, 'complex', 1); % todo: test adjoint for wi complex
